...
def make_model(num_ch_c1, num_ch_c2, use_dropout):
    ...
    # Add dropout layer with dropout rate of 0.5 depending on the paramter use_dropout
    if use_dropout:
...
def main():
    ...
    for i, img in enumerate(test_imgs):
        ...
        plt.savefig(f'./results/testImage_{i}_new.png')
    ...
    '''
    layer_outputs = [layer.output for layer in model.layers[1:5]]  # select the first 4 layers except the input layer
    activation_model = models.Model(model.input, layer_outputs)


    activations = activation_model.predict(test_imgs)
