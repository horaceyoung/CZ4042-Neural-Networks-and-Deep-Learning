...
# store model results
result = open(f'results_b/B1_accuracy_and_time_dropout_{dropout}.txt', mode='w+')
# last template value is the final accuracy result
result.writelines(f'Model from B1 used {end_time} seconds to run, and has evaluation result of {template}')
...
plt.xlabel('epoch')
plt.legend()
plt.savefig(f'./results_b/B1_Losses_dropout_{dropout}.png')
plt.close()
...
plt.xlabel('epoch')
plt.legend()
plt.savefig(f'./results_b/B1_Accuracies_dropout_{dropout}.png')
plt.close()
...
plt.ylabel('Cost')
plt.xlabel('epoch')
plt.savefig(f'./results_b/B1_train_cost_dropout_{dropout}.png')
plt.close()
...
plt.ylabel('Accuracy')
plt.xlabel('epoch')
plt.savefig(f'./results_b/B1_test_acc_dropout_{dropout}.png')
plt.close()
