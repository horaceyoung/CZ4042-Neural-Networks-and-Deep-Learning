...
class CharRNN(Model):
    def __init__(self, vocab_size, hidden_dim=20):
        ...
        self.rnn = layers.RNN(
            tf.keras.layers.GRUCell(self.hidden_dim), unroll=True)  # vanilla RNN Layer
...
optimizer = tf.keras.optimizers.Adam(learning_rate=lr, clipvalue=2) # Add clip value to 2
...
# store model results
result = open(f'results_b/B6_fromB3_(c)_accuracy_and_time_dropout_{dropout}.txt', mode='w+')
# last template value is the final accuracy result
result.writelines(f'Model from B6_fromB3_(c) used {end_time} seconds to run, and has evaluation result of {template}')
...
plt.xlabel('epoch')
plt.legend()
plt.savefig(f'./results_b/B6_fromB3_(c)_Losses_dropout_{dropout}.png')
plt.close()
...
plt.xlabel('epoch')
plt.legend()
plt.savefig(f'./results_b/B6_fromB3_(c)_Accuracies_dropout_{dropout}.png')
plt.close()
...
plt.ylabel('Cost')
plt.xlabel('epoch')
plt.savefig(f'./results_b/B6_fromB3_(c)_train_cost_dropout_{dropout}.png')
plt.close()
...
plt.ylabel('Accuracy')
plt.xlabel('epoch')
plt.savefig(f'./results_b/B6_fromB3_(c)_test_acc_dropout_{dropout}.png')
plt.close()
