...
class CharRNN(Model):
    ...
    def call(self, x, drop_rate):
        ...
        x = self.rnn1(x)
        x = self.rnn2(x) # additional RNN layer
...
# store model results
result = open(f'results_b/B6_fromB3_(b)_accuracy_and_time_dropout_{dropout}.txt', mode='w+')
# last template value is the final accuracy result
result.writelines(f'Model from B6_fromB3_(b) used {end_time} seconds to run, and has evaluation result of {template}')
...
plt.xlabel('epoch')
plt.legend()
plt.savefig(f'./results_b/B6_fromB3_(b)_Losses_dropout_{dropout}.png')
plt.close()
...
plt.xlabel('epoch')
plt.legend()
plt.savefig(f'./results_b/B6_fromB3_(b)_Accuracies_dropout_{dropout}.png')
plt.close()
...
plt.ylabel('Cost')
plt.xlabel('epoch')
plt.savefig(f'./results_b/B6_fromB3_(b)_train_cost_dropout_{dropout}.png')
plt.close()
...
plt.ylabel('Accuracy')
plt.xlabel('epoch')
plt.savefig(f'./results_b/B6_fromB3_(b)_test_acc_dropout_{dropout}.png')
plt.close()
